...
void BuddyAllocator_init(
    ...
    void* item0 =BuddyAllocator_malloc(b_alloc, 2000); 
...
void* BuddyAllocator_getBuddy(BuddyAllocator* b_alloc, DATA_MAX level){
    ...
    if(tree_buddiesOnLevel(b_alloc->tree, level)){
        ...
        DATA_MAX offset = b_alloc->min_bucket_size * (b_alloc->num_levels-level) * (idx);
        printf("[Offset]: %d\n", offset);
...
void* BuddyAllocator_malloc(BuddyAllocator* alloc, DATA_MAX size){
    ...
    printf("[MEM_SIZE]:%d\n",mem_size);
    DATA_MAX level = tree_level(alloc->tree, mem_size/(size+sizeof(void*)));
    printf("[LEVEL]:%d\n",level);
    ...
    void* buddy = BuddyAllocator_getBuddy(alloc, level);
