...
buddy:
	$(CC) $(CCOPTS) -o Bud_test Main.c Bitmap.* BuddyAllocator.* pool_allocator.* $(LIBS)
	./Bud_test
	mv Bud_test $(BINS_DIR)


buddy.o: 
	$(CC) $(CCOPTS) $(CC_DUMP) Main.c Bitmap.*  BuddyAllocator.* pool_allocator.* $(LIBS)
...
buddyGProf:
	$(CC) $(CCOPTS) $(CC_GPROF) -o Bud_test Main.c Bitmap.* * BuddyAllocator.* pool_allocator.* $(LIBS)
	./Bud_test
...
BitmapTest:
	$(CC) $(CCOPTS) -o BM_test $(TEST_DIR)Bitmap_test.c Bitmap.* pool_allocator.*
	./BM_test
	mv BM_test $(BINS_DIR)
